package Copilot::Component::JobManager::Panda;

=head1 NAME Copilot::Component::JobManager::Panda

=head1 DESCRIPTION

This class implements the JobManager of the Copilot system for Panda. It is a child class of Copilot::Component (for general information 
...
sub _loadConfig
{
    ...
    # chirp hostname 
    $self->{'CHIRP_HOST'} = `hostname -f`;
    chomp $self->{'CHIRP_HOST'};

   $self->{'CHIRP_PORT'} = 9094;
}
...
sub componentGetJobOutputDirHandler
{
	...
	`mkdir 	$jobOutDir`;
	`echo "hostname:* rwl" > $jobOutDir/.__acl`;
...
sub componentGetJobHandler
{
    ...
    my $jobGuid = $guid->CreateGuid();
    
    my $job = {};
    $job->{'id'} = $jobGuid;
    $job->{'chirpUrl'} = 'cvmappi10.cern.ch';
    $job->{'inputDir'} = '/panda';
    $job->{'inputFiles'} = 'startPanda###';
    $job->{'packages'} = '';
    $job->{'environment'} = '';
    $job->{'command'} = 'startPanda';
    $job->{'arguments'} = '';
